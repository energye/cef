...
func (m *TCefMediaSinkRef) GetId() string {
	if !m.IsValid() {
		return ""
	}
	r := cefMediaSinkRefAPI().SysCallN(1, m.Instance())
	return api.GoStr(r)
}

func (m *TCefMediaSinkRef) GetName() string {
	if !m.IsValid() {
		return ""
	}
	r := cefMediaSinkRefAPI().SysCallN(2, m.Instance())
	return api.GoStr(r)
}
