...
func (m *TCefCommandLineRef) GetCommandLineString() string {
	if !m.IsValid() {
		return ""
	}
	r := cefCommandLineRefAPI().SysCallN(4, m.Instance())
	return api.GoStr(r)
}

func (m *TCefCommandLineRef) GetProgram() string {
	if !m.IsValid() {
		return ""
	}
	r := cefCommandLineRefAPI().SysCallN(5, m.Instance())
	return api.GoStr(r)
}
...
func (m *TCefCommandLineRef) GetSwitchValue(name string) string {
	if !m.IsValid() {
		return ""
	}
	r := cefCommandLineRefAPI().SysCallN(8, m.Instance(), api.PasStr(name))
	return api.GoStr(r)
}
