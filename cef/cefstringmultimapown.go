...
func (m *TCefStringMultimapOwn) GetEnumerate(key string, valueIndex cefTypes.NativeUInt) (result string) {
	...
	cefStringMultimapOwnAPI().SysCallN(4, m.Instance(), api.PasStr(key), uintptr(valueIndex), sw.Instance())
	result = sw.Value()
	sw.Free()
	return
}
...
func (m *TCefStringMultimapOwn) GetKey(index cefTypes.NativeUInt) (result string) {
	...
	sw := lcl.NewStringWrap()
	cefStringMultimapOwnAPI().SysCallN(5, m.Instance(), uintptr(index), sw.Instance())
	result = sw.Value()
	sw.Free()
	return
}
...
func (m *TCefStringMultimapOwn) GetValue(index cefTypes.NativeUInt) (result string) {
	...
	sw := lcl.NewStringWrap()
	cefStringMultimapOwnAPI().SysCallN(6, m.Instance(), uintptr(index), sw.Instance())
	result = sw.Value()
	sw.Free()
	return
}
