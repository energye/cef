...
func (m *TCefPreferenceRegistrarRef) AddPreference(name string, defaultvalue ICefValue) bool {
	r1 := preferenceRegistrarRefImportAPI().SysCallN(0, m.Instance(), PascalStr(name), GetObjectUintptr(defaultvalue))
	return GoBool(r1)
}

var (
	preferenceRegistrarRefImport       *imports.Imports = nil
	preferenceRegistrarRefImportTables                  = []*imports.Table{
		/*0*/ imports.NewTable("CefPreferenceRegistrarRef_AddPreference", 0),
	}
)

func preferenceRegistrarRefImportAPI() *imports.Imports {
	if preferenceRegistrarRefImport == nil {
		preferenceRegistrarRefImport = NewDefaultImports()
		preferenceRegistrarRefImport.SetImportTable(preferenceRegistrarRefImportTables)
		preferenceRegistrarRefImportTables = nil
	}
	return preferenceRegistrarRefImport
}
