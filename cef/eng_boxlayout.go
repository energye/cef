...
func (m *boxLayout) UnWrap(data uintptr) ICefBoxLayout {
	...
	boxLayoutImportAPI().SysCallN(2, uintptr(data), uintptr(unsafePointer(&resultCefBoxLayout)))
	return AsCefBoxLayout(resultCefBoxLayout)
}

func (m *TCefBoxLayout) SetFlexForView(view ICefView, flex int32) {
	boxLayoutImportAPI().SysCallN(1, m.Instance(), GetObjectUintptr(view), uintptr(flex))
}

func (m *TCefBoxLayout) ClearFlexForView(view ICefView) {
	boxLayoutImportAPI().SysCallN(0, m.Instance(), GetObjectUintptr(view))
}

var (
	boxLayoutImport       *imports.Imports = nil
	boxLayoutImportTables                  = []*imports.Table{
		/*0*/ imports.NewTable("CefBoxLayout_ClearFlexForView", 0),
		/*1*/ imports.NewTable("CefBoxLayout_SetFlexForView", 0),
		/*2*/ imports.NewTable("CefBoxLayout_UnWrap", 0),
	}
)

func boxLayoutImportAPI() *imports.Imports {
	if boxLayoutImport == nil {
		boxLayoutImport = NewDefaultImports()
		boxLayoutImport.SetImportTable(boxLayoutImportTables)
		boxLayoutImportTables = nil
	}
	return boxLayoutImport
}
