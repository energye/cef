...
// ICefResponse Parent: ICefBaseRefCountedRef
...
// ICefResponseRef Parent: ICefResponse
...
func (m *TCefResponseRef) GetStatusText() string {
	if !m.IsValid() {
		return ""
	}
	r := cefResponseRefAPI().SysCallN(4, m.Instance())
	return api.GoStr(r)
}

func (m *TCefResponseRef) GetMimeType() string {
	if !m.IsValid() {
		return ""
	}
	r := cefResponseRefAPI().SysCallN(5, m.Instance())
	return api.GoStr(r)
}

func (m *TCefResponseRef) GetCharset() string {
	if !m.IsValid() {
		return ""
	}
	r := cefResponseRefAPI().SysCallN(6, m.Instance())
	return api.GoStr(r)
}

func (m *TCefResponseRef) GetHeaderByName(name string) string {
	if !m.IsValid() {
		return ""
	}
	r := cefResponseRefAPI().SysCallN(7, m.Instance(), api.PasStr(name))
	return api.GoStr(r)
}

func (m *TCefResponseRef) GetURL() string {
	if !m.IsValid() {
		return ""
	}
	r := cefResponseRefAPI().SysCallN(8, m.Instance())
	return api.GoStr(r)
}
