...
func (m *TCefCustomStringMultimap) GetEnumerate(key string, valueIndex cefTypes.NativeUInt) (result string) {
	...
	cefCustomStringMultimapAPI().SysCallN(4, m.Instance(), api.PasStr(key), uintptr(valueIndex), sw.Instance())
	result = sw.Value()
	sw.Free()
	return
}
...
func (m *TCefCustomStringMultimap) GetKey(index cefTypes.NativeUInt) (result string) {
	...
	sw := lcl.NewStringWrap()
	cefCustomStringMultimapAPI().SysCallN(5, m.Instance(), uintptr(index), sw.Instance())
	result = sw.Value()
	sw.Free()
	return
}
...
func (m *TCefCustomStringMultimap) GetValue(index cefTypes.NativeUInt) (result string) {
	...
	sw := lcl.NewStringWrap()
	cefCustomStringMultimapAPI().SysCallN(6, m.Instance(), uintptr(index), sw.Instance())
	result = sw.Value()
	sw.Free()
	return
}
