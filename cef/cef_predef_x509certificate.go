...
func (m *TCefX509Certificate) GetDEREncodedIssuerChain(chainCount *NativeUInt, chain *ICefBinaryValueArray) {
	var result1 uintptr
	predefImportAPI().SysCallN(5, m.Instance(), uintptr(unsafePointer(chainCount)), uintptr(unsafePointer(&result1)))
	if result1 > 0 {
...
func (m *TCefX509Certificate) GetPEMEncodedIssuerChain(chainCount *NativeUInt, chain *ICefBinaryValueArray) {
	var result1 uintptr
	predefImportAPI().SysCallN(6, m.Instance(), uintptr(unsafePointer(chainCount)), uintptr(unsafePointer(&result1)))
	if result1 > 0 {
		*chain = BinaryValueArrayRef.New(int(*chainCount), result1)
	}
}
