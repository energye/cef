...
func (m *TCefSchemeRegistrarRef) AddCustomScheme(schemeName string, options TCefSchemeOptions) bool {
	r1 := schemeRegistrarRefImportAPI().SysCallN(0, m.Instance(), PascalStr(schemeName), uintptr(options))
	return GoBool(r1)
}

var (
	schemeRegistrarRefImport       *imports.Imports = nil
	schemeRegistrarRefImportTables                  = []*imports.Table{
		/*0*/ imports.NewTable("CefSchemeRegistrarRef_AddCustomScheme", 0),
	}
)

func schemeRegistrarRefImportAPI() *imports.Imports {
	if schemeRegistrarRefImport == nil {
		schemeRegistrarRefImport = NewDefaultImports()
		schemeRegistrarRefImport.SetImportTable(schemeRegistrarRefImportTables)
		schemeRegistrarRefImportTables = nil
	}
	return schemeRegistrarRefImport
}
