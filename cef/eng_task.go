...
func NewTask() ITask {
	r1 := askImportAPI().SysCallN(0)
	return AsTask(r1)
}

func (m *TTask) OnlyPostTask(threadId TCefThreadId) bool {
	r1 := askImportAPI().SysCallN(2, m.Instance(), uintptr(threadId))
	return GoBool(r1)
}

func (m *TTask) OnlyPostDelayedTask(threadId TCefThreadId, delay int64) bool {
	r1 := askImportAPI().SysCallN(1, m.Instance(), uintptr(threadId), uintptr(unsafePointer(&delay)))
	return GoBool(r1)
}
...
func (m *TTask) SetOnExecute(fn TTaskExecute) {
	...
	askImportAPI().SysCallN(3, m.Instance(), m.executePtr)
}

var (
	askImport       *imports.Imports = nil
	askImportTables                  = []*imports.Table{
		/*0*/ imports.NewTable("Task_Create", 0),
		/*1*/ imports.NewTable("Task_OnlyPostDelayedTask", 0),
		/*2*/ imports.NewTable("Task_OnlyPostTask", 0),
		/*3*/ imports.NewTable("Task_SetOnExecute", 0),
	}
)
