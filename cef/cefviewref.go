...
func (m *TCefViewRef) GetTypeString() string {
	if !m.IsValid() {
		return ""
	}
	r := cefViewRefAPI().SysCallN(6, m.Instance())
	return api.GoStr(r)
}

func (m *TCefViewRef) ToStringEx(includeChildren bool) string {
	if !m.IsValid() {
		return ""
	}
	r := cefViewRefAPI().SysCallN(7, m.Instance(), api.PasBool(includeChildren))
	return api.GoStr(r)
}
