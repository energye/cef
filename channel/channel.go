...
import (
	...
	"encoding/binary"
	"errors"
	"github.com/energye/cef/cef/process"
...
type IIPCContext interface {
	...
	ChannelType() types.ChannelType // 返回 当前通道类型
	ProcessId() types.CefProcessId  // 返回 通道消息来源
	Message() IMessage              // 消息
	Free()                          //
}
...
type IPCContext struct {
	...
	toChannelId int64              //
	ipcType     types.IPC_TYPE     // ipc type
	channelType types.ChannelType  // ipc channel type
	processId   types.CefProcessId // ipc msg source, browser or render
...
func (m *IPCContext) ChannelType() types.ChannelType {
	return m.channelType
}

func (m *IPCContext) ProcessId() types.CefProcessId {
	return m.processId
}
...
func (m *channel) write(messageType mt, channelId, toChannelId int64, data []byte) (n int, err error) {
	...
	var processId types.CefProcessId
	if m.channelType == types.Ct_Server {
		processId = types.PID_BROWSER
	} else {
		processId = types.PID_RENDER
	}
	...
	for {
		...
		if size == headerLength {
			...
			m.handler(&IPCContext{
				...
				processId:   types.CefProcessId(proId),
