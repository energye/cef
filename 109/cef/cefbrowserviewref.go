...
func (m *TCefBrowserViewRef) SetPreferAccelerators(preferAccelerators bool) {
	...
	cefBrowserViewRefAPI().SysCallN(6, m.Instance(), api.PasBool(preferAccelerators))
}
...
func (_BrowserViewRefClass) UnWrapWithPointer(data uintptr) (result ICefBrowserView) {
	var resultPtr uintptr
	cefBrowserViewRefAPI().SysCallN(3, uintptr(data), uintptr(base.UnsafePointer(&resultPtr)))
...
func (_BrowserViewRefClass) CreateBrowserView(client IEngClient, url string, settings TCefBrowserSettings, extraInfo ICefDictionaryValue, requestContext ICefRequestContext, delegate IEngBrowserViewDelegate) (result ICefBrowserView) {
	...
	var resultPtr uintptr
	cefBrowserViewRefAPI().SysCallN(4, base.GetObjectUintptr(client), api.PasStr(url), uintptr(base.UnsafePointer(settingsPtr)), base.GetObjectUintptr(extraInfo), base.GetObjectUintptr(requestContext), base.GetObjectUintptr(delegate), uintptr(base.UnsafePointer(&resultPtr)))
	result = AsCefBrowserViewRef(resultPtr)
	return
}
...
func (_BrowserViewRefClass) GetForBrowser(browser ICefBrowser) (result ICefBrowserView) {
	var resultPtr uintptr
	cefBrowserViewRefAPI().SysCallN(5, base.GetObjectUintptr(browser), uintptr(base.UnsafePointer(&resultPtr)))
	result = AsCefBrowserViewRef(resultPtr)
	return
}
...
func cefBrowserViewRefAPI() *imports.Imports {
	cefBrowserViewRefOnce.Do(func() {
		...
		cefBrowserViewRefImport.Table = []*imports.Table{
			...
			/* 3 */ imports.NewTable("TCefBrowserViewRef_UnWrapWithPointer", 0), // static function UnWrapWithPointer
			/* 4 */ imports.NewTable("TCefBrowserViewRef_CreateBrowserView", 0), // static function CreateBrowserView
			/* 5 */ imports.NewTable("TCefBrowserViewRef_GetForBrowser", 0), // static function GetForBrowser
			/* 6 */ imports.NewTable("TCefBrowserViewRef_SetPreferAccelerators", 0), // procedure SetPreferAccelerators
		}
	})
